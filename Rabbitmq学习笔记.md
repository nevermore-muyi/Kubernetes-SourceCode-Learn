...
Exclusive：仅仅对声明它的连接可见（Connection），只允许被一个客户端使用，客户端断开连接即被删除；
Auto-delete：当所有消费者都取消订阅时，队列自动被删除；
ps：当一个Queue被多个消费者订阅时，消息只会被一个消费者消费。

队列模式有6种：
1.简单模式--Hello World模式：一个生产者发送消息到队列，一个消费者接收消息；
2.工作队列模式--Work Queue模式：一个生产者多个消费者。消费者共享队列，消息唯一；
3.发布/订阅模式：一个生产者，一个Exchange，多个队列，每个队列对应单独的消费者；
4.路由模式--Routing：生产者发送消息到Exchange上，Exchange通过路由匹配发送到相应的队列中；
5.通配符模式--Topic：与路由类似，使用的通配符匹配；
6.RPC模式：远程调用。
```
